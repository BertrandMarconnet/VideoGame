...
func _bootstrap_current_scene() -> void:
	...
	if scene_root == null or not is_instance_valid(scene_root):
		return
	_scene_instance_id = scene_root.get_instance_id()
...
func regenerate(seed_value: int) -> Dictionary:
	if scene_root == null or not is_instance_valid(scene_root):
		return {"error": "scene_unavailable"}
...
func _maintain_start_ui() -> void:
	...
	var state := "menu" if panel.visible and not game_started else "game"
	if state == _ui_state:
		return
	_ui_state = state
...
func _new_launch_seed() -> int:
	...
	var source := "%s|%d|%d|%s" % [
		...
		Time.get_ticks_usec(),
		str(get_tree().current_scene.scene_file_path if get_tree().current_scene != null else "runtime")
	]
...
func _load_or_create_installation_id() -> String:
	...
	var value := "%d-%d-%d" % [int(Time.get_unix_time_from_system()), Time.get_ticks_usec(), randi()]

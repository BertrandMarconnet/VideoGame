...
func _run() -> void:
	var database: RefCounted = MaterialResponseDBScript.new() as RefCounted
	var drywall_crowbar := float(database.call("damage_multiplier", "drywall", "crowbar"))
	var brick_crowbar := float(database.call("damage_multiplier", "brick", "crowbar"))
	var brick_charge := float(database.call("damage_multiplier", "brick", "specter_charge"))
	var brick_flashlight := float(database.call("damage_multiplier", "brick", "flashlight_bash"))
	_check(drywall_crowbar > brick_crowbar, "crowbar must be more effective on drywall than brick")
	_check(brick_charge > brick_flashlight, "SPECTER charge must outperform flashlight on brick")
	_check(bool(database.call("is_structural", "concrete")), "concrete must be structural")
	...
	var wall_component: Node = DestructibleComponentScript.new() as Node
	wall_component.name = "DestructibleComponent"
	wall.add_child(wall_component)
	await process_frame
	wall_component.call("configure", {
		"category": "wall",
	...
	var wall_result: Dictionary = wall_component.call("apply_damage", {
		"zone_id": "cell_00_00",
		"amount": 10.0,
		"tool_id": "crowbar",
		"damage_type": "impact"
	...
	var specter_component: Node = DestructibleComponentScript.new() as Node
	specter_component.name = "DestructibleComponent"
	specter.add_child(specter_component)
	await process_frame
	specter_component.call("configure", {
	...
	specter_component.call("apply_damage", {"zone_id":"left_leg","amount":50.0,"tool_id":"crowbar","damage_type":"impact"})
	specter_component.call("apply_damage", {"zone_id":"right_leg","amount":50.0,"tool_id":"crowbar","damage_type":"impact"})
	await process_frame
	_check(String(specter_component.get("movement_mode")) == "crawl", "SPECTER must switch to crawl after both legs break")
	_check(float(specter_component.get("speed_multiplier")) < 0.5, "two broken legs must substantially reduce movement speed")

...
## Génération Meshy depuis GitHub Actions

Le dépôt contient désormais un workflow manuel :

```text
.github/workflows/generate-meshy-asset.yml
```

Il utilise `tools/meshy_generate.py` pour valider un manifeste, appeler l’API Meshy, attendre la génération, télécharger le GLB et produire un rapport de provenance. Le résultat reste un artifact GitHub destiné à une revue manuelle ; il n’est jamais commité automatiquement.

Documentation complète : `docs/MESHY_GITHUB_PIPELINE.md`.

Le workflow dispose de deux modes :

- `validate` : contrôle local sans appel API et sans consommation de crédits ;
- `generate` : appel Meshy utilisant le secret GitHub `MESHY_API_KEY`.

Le premier manifeste préparé concerne SPECTER-5 :

```text
assets/concepts/specter_5/meshy.json
```

Les quatre images de référence doivent être ajoutées avant la première validation. Tout modèle généré doit ensuite passer par Blender et les validations Godot/Web/mobile décrites ci-dessus.
